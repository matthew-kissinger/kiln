...
export function fingerprintRendererInputs(roots, gpuState) {
  ...
  for (let i = 0; i < roots.length; i++) {
    const root = realpathSync(roots[i]);
    const visit = (path) => {
      if (statSync(path).isDirectory()) {
        ...
        hash.update(JSON.stringify([i, relative(root, path).replaceAll('\\', '/'), bytes.length]));
        hash.update(bytes);
      }
    };
    visit(root);
...
function packageRoot(name) {
  ...
  for (;;) {
    ...
    if (parent === dir || dir === parse(dir).root)
      throw new Error(`Missing installed renderer dependency ${name}`);
    dir = parent;
  }
}
export function createRendererCaptureIdentity(gpuState) {
  try {
    return Object.freeze({
      version: 'kiln.capture-producer.v1',
      fingerprint: fingerprintRendererInputs(
        [
          fileURLToPath(new URL('.', import.meta.url)),
          ...['three', 'webgpu', 'pngjs'].map(packageRoot),
        ],
        gpuState,
      ),
