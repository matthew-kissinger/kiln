...
function build() {
  ...
  // Museum catalog plaque on front face (+X face)
  createPart('MuseumPlaque', boxGeo(0.006, 0.07, 0.16), polishedBronzeMat, {
    position: [0.303, 0.20, 0],
    parent: root,
  });
  ...
  for (let i = 0; i < NUM_FINS; i++) {
    ...
    // Coherent Progressive Silhouette Parameters
    // Wave height rises to an asymmetric crest around u = 0.65 then tapers
    const height = 0.42 + 0.52 * Math.sin(Math.PI * Math.pow(u, 0.8)) * (1.0 - 0.2 * Math.pow(u - 0.65, 2));
    const sweepX = 0.08 + 0.22 * Math.pow(u, 1.2);
    const archZ = 0.11 * Math.sin(Math.PI * u) * (1.0 - 0.25 * u);
    const twistDeg = -10.0 + 26.0 * u;
    const chordBase = 0.062 + 0.025 * Math.sin(Math.PI * u);
    const thicknessBase = 0.016;
    ...
    // Structural bronze spine / reinforcing clasp along lower blade
    const spineHeight = height * 0.32;
    createPart(`Spine_${i}`, boxGeo(0.016, spineHeight, 0.022), polishedBronzeMat, {
      position: [0.006, 0.04 + spineHeight * 0.5, 0],
      rotation: [0, 0, -4 * (u - 0.5)],
      parent: pivot,
    });
  }
