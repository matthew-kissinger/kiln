...
function build() {
  ...
  for (let f = 0; f < 4; f++) {
    ...
    createPart('RadiatorFin_' + f, boxGeo(0.38, 0.12, 0.015), hullIvory, {
      position: [-0.65, 0, 0],
      rotation: [fDeg, 0, 0],
      parent: coreRoot
    });
    createPart('RadiatorFinTrim_' + f, boxGeo(0.40, 0.02, 0.02), darkTitanium, {
      position: [-0.65, 0, 0],
      rotation: [fDeg, 0, 0],
      parent: coreRoot
    });
  ...
  for (let k = 0; k < 4; k++) {
    ...
    beamBetween('InnerBatten_' + k, [0, 0.10, 0], [0, 1.34, 0], 0.008, darkTitanium, {
      parent: sailInnerJoint
    });
    ...
    beamBetween('OuterBatten_' + k, [0, 0.02, 0], [0, 1.35, 0], 0.007, darkTitanium, {
      parent: sailOuterJoint
    });
    ...
    const outerTipY = 1.35 * Math.cos(1.22);
    const outerTipZ = 1.35 * Math.sin(1.22);
