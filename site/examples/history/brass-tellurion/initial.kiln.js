...
function build() {
  ...
  const land = gameMaterial(0x7a6a3f, { metalness: 0.1, roughness: 0.8 });
  ...
  for (let i = 0; i < footPos.length; i++) {
    createPart('Foot' + i, sphereGeo(0.045, 16, 12), brassDark, { position: [footPos[i][0], 0.028, footPos[i][1]], parent: root });
  }
  ...
  const gearA = createPivot('GearA', [0.13, 0.20, 0.14], root);
  createPart('GearAMesh', gearGeo({ teeth: 36, rootRadius: 0.085, tipRadius: 0.10, boreRadius: 0.014, height: 0.028 }), brass, { parent: gearA });
  createPart('GearAHub', cylinderGeo(0.016, 0.016, 0.07, 16), steel, { parent: gearA });
  createPart('GearACap', sphereGeo(0.02, 12, 8), brassBright, { position: [0, 0.045, 0], parent: gearA });

  const gearB = createPivot('GearB', [-0.045, 0.20, 0.175], root);
  createPart('GearBMesh', gearGeo({ teeth: 24, rootRadius: 0.056, tipRadius: 0.067, boreRadius: 0.012, height: 0.028 }), brassBright, { parent: gearB });
  createPart('GearBHub', cylinderGeo(0.013, 0.013, 0.07, 12), steel, { parent: gearB });

  const gearC = createPivot('GearC', [-0.155, 0.20, 0.075], root);
  createPart('GearCMesh', gearGeo({ teeth: 18, rootRadius: 0.042, tipRadius: 0.051, boreRadius: 0.010, height: 0.028 }), brassDark, { parent: gearC });
  createPart('GearCHub', cylinderGeo(0.011, 0.011, 0.07, 12), steel, { parent: gearC });

  // small vertical pinion driving gear A from below crank shaft
  const pinion = createPivot('Pinion', [0.235, 0.20, 0.03], root);
  ...
  beamBetween('CrankShaft', [0.235, 0.20, 0.03], [0.47, 0.20, 0.03], 0.011, steel, { parent: root });
  const crank = createPivot('Crank', [0.47, 0.20, 0.03], root);
  ...
  beamBetween('CrankPost', [0.44, 0.03, 0.03], [0.44, 0.20, 0.03], 0.014, brassDark, { parent: root });
  ...
  const conts = [
    { p: [0.10, 0.08, 0.16], s: [0.10, 0.05, 0.13], r: [20, 30, 10] },
    { p: [-0.13, 0.02, 0.14], s: [0.08, 0.06, 0.10], r: [-10, -40, 0] },
    { p: [-0.02, -0.10, -0.18], s: [0.11, 0.05, 0.09], r: [30, 10, -20] },
    { p: [0.02, 0.16, -0.12], s: [0.09, 0.04, 0.08], r: [0, 20, 30] },
    { p: [-0.16, -0.08, -0.06], s: [0.06, 0.04, 0.07], r: [10, 0, 40] }
  ];
  ...
  beamBetween('PhaseRod', [-0.02, 1.12, -0.02], [0.40, 1.24, 0.10], 0.005, steel, { parent: root });
  ...
  createPart('Plaque', boxGeo(0.20, 0.05, 0.012), brassDark, { position: [0.10, 0.10, 0.435], rotation: [-12, 0, 0], parent: root });
  createPart('PlaqueFace', boxGeo(0.18, 0.036, 0.004), ivory, { position: [0.10, 0.103, 0.441], rotation: [-12, 0, 0], parent: root });
