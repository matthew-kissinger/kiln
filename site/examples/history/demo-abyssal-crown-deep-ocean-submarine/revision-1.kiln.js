...
async function build() {
  ...
  const sailBody = await roundedBoxGeo(2.55, 1.48, 0.78, 0.20, { segments: 8, smooth: true });
  createPart('CommandSail', sailBody, charcoal, {
    position: [0.48, 4.15, 0], rotation: [0, 0, -3], parent: root
  });
  const sailCap = await roundedBoxGeo(1.92, 0.12, 0.66, 0.055, { segments: 6, smooth: true });
  createPart('SailCap', sailCap, pale, {
    position: [0.46, 4.90, 0], rotation: [0, 0, -3], parent: root
  });

  const mastPositions = [
    ['AttackPeriscope', 0.05, 5.43, -0.08, 0.075, 0.98],
    ['SearchPeriscope', 0.68, 5.36, 0.12, 0.065, 0.78],
    ['SensorMast', -0.48, 5.28, 0.10, 0.055, 0.62]
  ];
  ...
  for (const side of [-1, 1]) {
    ...
    createPart((side > 0 ? 'Starboard' : 'Port') + '_SailPlaque',
      await roundedBoxGeo(0.78, 0.16, 0.035, 0.012, { segments: 5 }), brass, {
        position: [0.55, 4.20, side * 0.405], parent: root
      });
  }
