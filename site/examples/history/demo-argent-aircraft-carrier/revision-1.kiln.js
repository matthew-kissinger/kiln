...
function build(){
 ...
 const orange=gameMaterial(0xd46b35,{roughness:.6});
 ...
 box('IslandOrangeRecognitionBand',5.7,.18,1.32,-.3,1.7,0,orange,island);
 ...
 function jet(n,x,z,angle){
  ...
  for(const s of [-1,1]){
   poly('SweptWing'+s,[[-.85,s*.16],[.4,s*.16],[-.44,s*1.07],[-.97,s*1.02]],.42,.055,steel,j);
   poly('Tailplane'+s,[[-1.12,s*.14],[-.62,s*.15],[-.95,s*.59],[-1.3,s*.54]],.48,.045,light,j);
   // Swept, outward-canted twin stabilizers.
   const fg=new THREE.BufferGeometry();
   const v=[-1.09,.49,s*.20,-.57,.49,s*.20,-.90,1.04,s*.34,-1.12,1.04,s*.34];
   fg.setAttribute('position',new THREE.Float32BufferAttribute(v,3));fg.setIndex([0,1,2,0,2,3,2,1,0,3,2,0]);fg.computeVertexNormals();createPart('VerticalStabilizer'+s,fg,navy,{parent:j});
  ...
  box('CanopySpine',.025,.015,.29,.36,.73,0,steel,j);
  return j;
 }
