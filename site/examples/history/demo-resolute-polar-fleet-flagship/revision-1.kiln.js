...
const DESIGN = { length: 32, beam: 6.6, deckY: 2.8, mastHeight: 9.3, radarWidth: 1.5, railHeight: 0.40 };
function build() {
 ...
 // Side portholes and projecting armour ribs give the hull scale.
 for(const s of [-1,1])for(let x=-11;x<=10;x+=1.05){
  part('Brass scuttle '+s+' '+x,torusGeo(.095,.024,6,12),brass,[x,2.28,s*3.13]);
  part('Dark scuttle glass '+s+' '+x,cylinderGeo(.078,.078,.018,12),black,[x,2.28,s*3.135],[90,0,0]);
 }
 ...
 function turret(n,x,y,dir,scale=1){
  ...
  box(n+' rangefinder',[.25*scale,.22*scale,2.15*scale],[-.45*scale,1.43*scale,0],brass,[0,0,0],g);
 }
 ...
 // Mast initially a compact pole radar; revise after render review.
 ...
 box('Radar panel',[.15,.6,DESIGN.radarWidth],[-1.25,DESIGN.mastHeight,0],blue);
 // REVISION_INSERT
 ...
 for(const s of [-1,1]){
  ...
  for(let i=0;i<12;i++)part('Anchor chain link '+s+' '+i,torusGeo(.07,.018,6,10),black,[12.25+i*.18,2.88,s*(.65+i*.015)],[90,i%2*60,0]);
