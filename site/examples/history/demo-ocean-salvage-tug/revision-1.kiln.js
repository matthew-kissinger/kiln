...
function build() {
  ...
  createPart('AFrameLegPort', cylinderGeo(0.07, 0.09, 1.8, 12), mats.orange, {
    position: [frameX, 1.95, -0.62], rotation: [20, 0, 0], parent: root,
  });
  createPart('AFrameLegStbd', cylinderGeo(0.07, 0.09, 1.8, 12), mats.orange, {
    position: [frameX, 1.95, 0.62], rotation: [-20, 0, 0], parent: root,
  });
  // Crossbeam between apexes + top pulley block.
  createPart('AFrameBeam', cylinderGeo(0.07, 0.07, 0.75, 12), mats.orange, {
    position: [frameX, 2.78, 0], rotation: [90, 0, 0], parent: root,
  });
  createPart('CraneBlock', boxGeo(0.18, 0.24, 0.14), mats.dark, {
    position: [frameX, 2.6, 0], parent: root,
  });
  // Hook cable + hook (hanging aft of the transom, under the block).
  createPart('HookCable', cylinderGeo(0.02, 0.02, 0.9, 8), mats.cable, {
    position: [frameX - 0.05, 2.05, 0], parent: root,
  });
  createPart('Hook', torusGeo(0.09, 0.03, 8, 16), mats.steel, {
    position: [frameX - 0.05, 1.55, 0], parent: root,
  });
