...
function build(){
 ...
 const crystal=gameMaterial(0x77c9c3,{metalness:0.25,roughness:0.3,emissive:0x2e6b67,emissiveIntensity:0.3});
 ...
 for(let i=0;i<7;i++){
  const a=i*2.4,r=2.2+(i%3)*0.27;
  const o=part('UndersideCrystal_'+i,new THREE.CylinderGeometry(0.03,0.18+(i%2)*0.08,0.95+(i%3)*0.27,5),crystal,[Math.cos(a)*r,2.08+(i%3)*0.3,Math.sin(a)*r*0.88],island);o.rotation.z=Math.PI+0.22*Math.sin(a);o.rotation.x=0.2*Math.cos(a);
 }
 for(let i=0;i<3;i++){
