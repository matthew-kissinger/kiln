...
function build(){
 ...
 const cyl=(n,rt,rb,h,m,p,r)=>add(n,new THREE.CylinderGeometry(rt,rb,h,32),m,p,r);
 const sphere=(n,s,m,p)=>createPart(n,new THREE.SphereGeometry(1,40,24),m,{position:p,scale:s,parent:root});
 const line=(n,pts,r,m)=>add(n,new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts.map(p=>new THREE.Vector3(...p))),Math.max(8,pts.length*4),r,6,false),m);
 ...
 const geo=new THREE.LatheGeometry(samples.map(p=>new THREE.Vector2(p.y,p.x)),80);
 ...
 // Conformal lower coating follows the pressure hull.
 const low=new THREE.LatheGeometry(samples.map(p=>new THREE.Vector2(p.y+.016,p.x)),80,0,Math.PI);
 ...
 [-39,-30,-18,-5,8,27,38,45].forEach((x,i)=>add('Hull_Expansion_Seam_'+i,new THREE.TorusGeometry(radius(x)+.008,.027,6,80),seam,[x,P.cy,0],[0,90,0]));
 function foil(chord,width,cx=0){const p=[];for(let i=0;i<=32;i++){let t=i/32;let z=5*width*(.2969*Math.sqrt(t)-.126*t-.3516*t*t+.2843*t*t*t-.1036*t*t*t*t);p.push([cx+chord/2-chord*t,z]);}for(let i=31;i>0;i--){let t=i/32;let z=5*width*(.2969*Math.sqrt(t)-.126*t-.3516*t*t+.2843*t*t*t-.1036*t*t*t*t);p.push([cx+chord/2-chord*t,-z]);}return p;}
 ...
 add('Sonar_Cap_Seam',new THREE.TorusGeometry(radius(43)+.023,.045,6,80),seam,[43,6.2,0],[0,90,0]);
 for(const s of [-1,1]){
