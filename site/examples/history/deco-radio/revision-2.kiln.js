...
async function build() {
  ...
  [-0.040, -0.028, -0.016, -0.004, 0.008, 0.020, 0.032, 0.044].forEach((zPos, idx) => {
    const warpGeo = cylinderYGeo(0.0008, 0.0008, 0.114, 8);
    createPart(`GrilleWarp_${idx}`, warpGeo, darkWoodMat, { position: [0.0910, 0.275, zPos], parent: root });
  });

  [-0.045, -0.030, -0.015, 0, 0.015, 0.030, 0.045].forEach((yOff, idx) => {
    const weftGeo = boxGeo(0.0016, 0.0016, 0.102);
  ...
  for (let f = 0; f < fluteCount; f++) {
    ...
    const fluteGeo = cylinderXGeo(0.0014, 0.0014, 0.012, 10);
